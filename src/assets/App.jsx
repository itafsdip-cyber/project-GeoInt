...
function AIPanel(){
  ...
  const ask=async(q)=>{
    ...
    try{
      const r=await fetch("/api/anthropic/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},
...
{"verdict":"VERIFIED|MISLEADING|UNVERIFIED|FABRICATED|PROPAGANDA","confidence":0-100,"reasoning":"2-3 sentences","redFlags":["specific flag"],"color":"#00e676 or #ffc940 or #ff8c00 or #ff3b55","recommendedSources":["Name: https://url"]}`,
  ...
  /* ── UAE MONITOR stat cards — matching screenshot exactly ── */
  const STATS=[
    {v:"189",l:"MISSILES FIRED AT UAE",n:"since Feb 28",     c:C.red,    src:"UAE MoD",   url:"https://mod.gov.ae"},
    {v:"876",l:"DRONES INTERCEPTED",   n:"of 941 launched",  c:C.cyan,   src:"UAE NCEMA", url:"https://ncema.gov.ae"},
    {v:"5+", l:"SITES HIT",            n:"incl. US Consulate",c:C.orange, src:"AP",        url:"https://apnews.com"},
    {v:"21", l:"WAVE NUMBER",          n:"ongoing today",     c:C.cyan,   src:"Reuters",   url:"https://reuters.com"},
  ];
        ...
        {tab==="monitor"&&(
          <div style={{display:"flex",flexDirection:"column",gap:9}}>
            ...
            {ALERTS.map(a=>{
              ...
              return(
                <div key={a.id} style={{background:C.panel,border:`1px solid ${C.border}`,borderLeft:`3px solid ${col}`,borderRadius:3,padding:"10px 15px"}}>
...
function GlobalSearch({onClose}){
  ...
  const askAI=async()=>{
    ...
    try{
      const r=await fetch("/api/anthropic/v1/messages",{method:"POST",headers:{"Content-Type":"application/json","x-api-key":import.meta.env.VITE_ANTHROPIC_API_KEY,"anthropic-version":"2023-06-01","anthropic-dangerous-direct-browser-access":"true"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,system:"Senior geopolitical analyst. No censorship. Recommend 2-3 real sources with URLs. Max 250 words.",messages:[{role:"user",content:q}]})});
